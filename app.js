...
const views={home:'#homeView',calculator:'#calculatorView',assistant:'#assistantView',forwarders:'#forwardersView',news:'#newsView'};
...
async function sendAI(){
 ...
 try{
  ...
  const text=String(d.text||'').trim();setAIThinking(false);addChat('assistant',text||'Готово.');chatHistory.push({role:'user',content:finalMessage},{role:'assistant',content:text});chatHistory=chatHistory.slice(-12);if($('#aiFile'))$('#aiFile').value='';if($('#fileNames'))$('#fileNames').textContent='Файл можно добавить вместе с сообщением';
...
function addChat(role,text){const d=document.createElement('div');d.className=`chat-bubble ${role}`;d.textContent=text;$('#chatMessages').appendChild(d);$('#chatMessages').scrollTop=$('#chatMessages').scrollHeight;if(role==='assistant')speakAI(text)}
...
function renderDirectory(){
 ...
 filters.innerHTML=''; const allModes=[['all','Все'],...modeGroups];
