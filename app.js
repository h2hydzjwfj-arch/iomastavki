...
$$('[data-close-view]').forEach(b=>b.addEventListener('click',()=>showView('home')));document.querySelector('#tarotView')?.addEventListener('click',e=>{if(e.target.id==='tarotView'||e.target.id==='tarotShell')showView('home')});
...
function setAIThinking(on,error=''){const s=$('#assistantStatus');if(!s)return;if(on){s.innerHTML='<span>'+tr('thinking')+'</span>';s.classList.add('thinking-active')}else{s.innerHTML=error?`<span>${escapeHtml(error)}</span>`:'';s.classList.remove('thinking-active')}}
