...
function prefetchNews(){if(newsCache.length)return Promise.resolve(newsCache);if(newsPrefetchPromise)return newsPrefetchPromise;newsPrefetchPromise=fetch('/api/news',{cache:'no-store',credentials:'same-origin'}).then(r=>{if(!r.ok)throw new Error('news');return r.json()}).then(d=>{newsCache=Array.isArray(d.items)?d.items:[];return newsCache}).catch(()=>newsCache||[]);return newsPrefetchPromise}
prefetchNews();
...
function closeTarot(){const shell=$('#tarotShell');if(!shell)return window.__iomaCloseViews?.();shell.classList.add('tarot-closing');try{startTarotDust(true)}catch{}setTimeout(()=>{shell.classList.remove('tarot-closing','tarot-ready');card.classList.remove('tarot-reveal-stable');window.__iomaCloseViews?.()},420)}
