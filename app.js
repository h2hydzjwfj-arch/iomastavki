...
const state={theme:localStorage.getItem('theme')||'light',chat:[],weather:null,cbr:null,voice:false,sound:true};
...
function closeModal(){modal.classList.add('hidden');body.innerHTML='';window.speechSynthesis&&speechSynthesis.cancel()}
...
/* ---------- AI-ассистент: текст + голос + озвучка ---------- */
function openAssistant(){
 openModal(modalHead('ИНТЕЛЛЕКТ','AI-ассистент','Логистика, расчёты и ВЭД — текстом или голосом')+`<div class="assistant"><div id="chat" class="chat"></div><div class="composer"><button id="mic" title="Диктовка: скажите вопрос — текст отправится автоматически">🎙</button><button id="snd" title="Озвучка ответов">${state.sound?'🔊':'🔇'}</button><textarea id="msg" placeholder="Напишите вопрос…"></textarea><button class="send" id="send">Отправить</button></div></div>`);
 ...
 $('#snd').onclick=()=>{state.sound=!state.sound;$('#snd').textContent=state.sound?'🔊':'🔇';if(!state.sound)speechSynthesis.cancel()};
}
...
function voiceInput(){
 ...
 r.onstart=()=>{state.voice=true;$('#mic').textContent='👂';toast('Слушаю… говорите вопрос')};
 r.onerror=e=>{state.voice=false;$('#mic').textContent='🎙';toast(e.error==='not-allowed'?'Разрешите доступ к микрофону':'Не удалось распознать речь')};
 r.onend=()=>{state.voice=false;const m=$('#mic');if(m)m.textContent='🎙'};
...
function speak(text){
 ...
 const u=new SpeechSynthesisUtterance(text.replace(/https?:\/\/\S+/g,'ссылка'));
 u.lang='ru-RU';u.rate=.98;
 const v=speechSynthesis.getVoices().find(v=>/ru/i.test(v.lang));if(v)u.voice=v;
...
/* ---------- ставки: просмотр + обновление через AI ---------- */
function openRates(){
