...
async function openArticle(n){
 ...
 const L=lang==='ru'?{what:'Что произошло',impact:'Что это меняет для логистики',scheme:'Схема влияния',law:'Правовой статус',watch:'Что проверить перед отправкой',source:'Источник',note:'Материал структурирован автоматически. Для юридически значимых решений проверяйте первоисточник.'}:lang==='en'?{what:'What happened',impact:'What it means for logistics',scheme:'Impact map',law:'Legal status',watch:'What to check before shipment',source:'Source',note:'This article is structured automatically. Check the primary source for legally significant decisions.'}:lang==='tr'?{what:'Ne oldu',impact:'Lojistik için ne anlama geliyor',scheme:'Etki şeması',law:'Hukuki durum',watch:'Gönderiden önce kontrol edin',source:'Kaynak',note:'Bu makale otomatik olarak yapılandırılmıştır. Hukuki açıdan önemli kararlar için birincil kaynağı kontrol edin.'}:{what:'发生了什么',impact:'对物流意味着什么',scheme:'影响图',law:'法律状态',watch:'发货前检查',source:'来源',note:'本文自动整理。涉及法律的重要决定请核对原始来源。'};
 const flow=lang==='ru'?['Изменение','Документы / тариф','Таможня','Срок / стоимость']:lang==='en'?['Change','Documents / tariff','Customs','Time / cost']:lang==='tr'?['Değişiklik','Belgeler / vergi','Gümrük','Süre / maliyet']:['变化','文件 / 关税','海关','时间 / 成本'];
 const status=urgent?(lang==='ru'?'СРОЧНО · ЗАКОН / ТАМОЖНЯ':lang==='en'?'URGENT · LAW / CUSTOMS':lang==='tr'?'ACİL · HUKUK / GÜMRÜK':'紧急 · 法律 / 海关'):(lang==='ru'?'ЛОГИСТИКА / РЫНОК':lang==='en'?'LOGISTICS / MARKET':lang==='tr'?'LOJİSTİK / PİYASA':'物流 / 市场');
