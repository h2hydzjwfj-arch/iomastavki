...
async function fetchGeo(q,target,box,input){try{const endpoint=q.length>=4?`/api/geocode?q=${encodeURIComponent(q)}&scope=${target}`:`/api/cities?q=${encodeURIComponent(q)}&country=${/^(asia|china)$/i.test(String(target||''))?'ASIA':'RU'}`;const r=await fetch(endpoint);const data=await r.json();if(input.value.trim()!==q)return;const remote=(data.results||[]).map(x=>{const label=x.display_name||x.name;const name=x.name||label.split(',')[0];const arr=[name,x.nameEn||name,x.nameZh||name,x.admin1||'',x.country||'',x.country_code,x.latitude,x.longitude];if(label&&label!==name)arr[8]={address:label};return arr});
...
async function loadNews(){
  ...
  const items=await prefetchNews(); if(!items.length){const fallback=[{title:'Китай → Россия: что проверить перед расчётом ставки',date:new Date().toISOString(),source:'IOMASTAVKA',description:'Incoterms, ТН ВЭД, габариты, объёмный вес, документы и терминальные расходы.',link:''},{title:'ТН ВЭД: какие данные нужны логисту до запроса ставки',date:new Date().toISOString(),source:'IOMASTAVKA',description:'Описание товара, код, инвойс, упаковка, страна происхождения и разрешительные документы.',link:''},{title:'ЖД, море или авиа: как выбрать перевозку из Китая',date:new Date().toISOString(),source:'IOMASTAVKA',description:'Сравнение сроков, расчётного веса и структуры стоимости.',link:''}];newsCache=fallback;renderNewsItems(fallback);return} renderNewsItems(items);
}
