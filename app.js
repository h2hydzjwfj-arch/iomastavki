...
$('#logoutButton').onclick=async()=>{await fetch('/api/logout',{method:'POST'});location.href='/'}; $('#lightThemeButton').onclick=()=>setTheme('light'); $('#darkThemeButton').onclick=()=>setTheme('dark'); setTheme(localStorage.getItem('iomastavka_theme')||'light');
...
function setTheme(mode){document.body.classList.toggle('manual-dark',mode==='dark');document.body.classList.toggle('manual-light',mode==='light');localStorage.setItem('iomastavka_theme',mode);}
...
function updateAutoVolume(){const [l,w,h]=dimensionsMm(),pieces=Number($('#pieces').value)||1,volume=(l*w*h/1e9)*pieces,factor=selectedMode&&modes[selectedMode]?modes[selectedMode].factor:167;const v=$('#autoVolume'),vw=$('#autoVolumetricWeight'),f=$('#autoFactor');if(v)v.textContent=volume?volume.toFixed(3)+' m³':'0 m³';if(vw)vw.textContent=volume?(volume*factor).toFixed(1)+' kg':'0 kg';if(f)f.textContent=factor+' kg/m³';}
