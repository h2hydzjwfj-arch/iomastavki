...
print('OK: заменим блок с ' + str(start) + ' по ' + str(end))

# Свежий чистый блок — без ошибок со скобками
...
function stripSourceFromTitle(t){
  ...
  x = x.replace(/\s+[-–—]\s+(?:Правда\.?Ру|Альта[-\s]?Софт|Логирус|ТАСС|РБК|Интерфакс|Delo\.ua|Alfa-Soft|Logirus|TKS\.RU|Portnews|Trans\.ru|Biznes Online|Eastrussia|Vgudok|AsiaRussia|Inbusiness\.kz|dknews\.kz|Правда\s*\.?\s*Ру|Бизнес\s*Online)[\s"']*$/i, '');
  return x.trim();
}
...
open_count = s.count('{')
close_count = s.count('}')
print('Скобки после замены: { = ' + str(open_count) + ', } = ' + str(close_count))
if open_count == close_count:
    ...
    if diff > 0:
        ...
        print('⚠️ Лишних } : ' + str(-diff) + ' — применим грубый фикс')
